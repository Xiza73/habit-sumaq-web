...
const activeBudget: BudgetWithKpi = {
  ...
  recovery: { zeroSpendDays: 0, partialSpend: null },
...
describe('BudgetKpiCard — recovery plan', () => {
  it('says nothing when there is nothing to recover', () => {
    renderCard({ recovery: { zeroSpendDays: 0, partialSpend: null } });
    expect(screen.queryByText(/recuperar/i)).not.toBeInTheDocument();
  });

  it('offers the half-spend plan when it fits', () => {
    renderCard({ recovery: { zeroSpendDays: 6, partialSpend: { fraction: 'HALF', days: 12 } } });
  ...
  it('offers a third when the half-spend plan does not fit', () => {
    renderCard({ recovery: { zeroSpendDays: 7, partialSpend: { fraction: 'THIRD', days: 11 } } });

    expect(screen.getByText(/7 días sin gastar/i)).toBeInTheDocument();
    expect(screen.getByText(/11 días gastando un tercio/i)).toBeInTheDocument();
    expect(screen.queryByText(/la mitad/i)).not.toBeInTheDocument();
  });

  it('offers a quarter as the last rung', () => {
    renderCard({ recovery: { zeroSpendDays: 9, partialSpend: { fraction: 'QUARTER', days: 12 } } });

    expect(screen.getByText(/12 días gastando un cuarto/i)).toBeInTheDocument();
  });

  it('drops the clause entirely when no fraction fits', () => {
    renderCard({ recovery: { zeroSpendDays: 10, partialSpend: null } });

    expect(screen.getByText(/10 días sin gastar/i)).toBeInTheDocument();
    expect(screen.queryByText(/gastando/i)).not.toBeInTheDocument();
  });

  it('uses the singular for a single day', () => {
    renderCard({ recovery: { zeroSpendDays: 1, partialSpend: { fraction: 'HALF', days: 2 } } });

    expect(screen.getByText(/1 día sin gastar/i)).toBeInTheDocument();
    expect(screen.queryByText(/1 días/i)).not.toBeInTheDocument();
  });

  it('says the month cannot be recovered when not even zero-spend fits', () => {
    renderCard({ recovery: { zeroSpendDays: null, partialSpend: null } });
