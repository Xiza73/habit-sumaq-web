 ...
 * and reset on reload, mirroring the design call we locked in for this
 * pre-release polish round.
 *
 * - `name`:        Locale-aware compare on `displayName`. Ties never expected
 *                   in practice (display names tend to be unique per user)
 *                   but the helper is stable so equal names stay in input
 *                   order.
 * - `netOwed`:     Sorted by the ABSOLUTE value — both "you owe a lot" and
 *                   "you're owed a lot" surface at the top in `desc`. The
 *                   sign (DEBT vs LOAN) is visible in the card itself.
 * - `pendingCount`: Numeric. Ties broken by `displayName` asc so the order
 *                   is deterministic when many rows share the same count.
 */
export function sortDebtRows(rows: DebtsSummaryRow[], prefs: DebtsViewPrefs): DebtsSummaryRow[] {
  const copy = [...rows];
  ...
  copy.sort((a, b) => {
    ...
    // Tie-breaker: displayName ASC so the order stays deterministic. We
    // don't flip this by `dirFactor` on purpose — the tie-breaker is for
    // stability, not user intent.
