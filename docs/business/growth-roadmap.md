...
| F4b | No aparece "abrir al iniciar máquina"         | web  | Falla en **runtime**, no por código faltante: capabilities `autostart:allow-*` presentes en el tag, `AutostartSection` montada en `settings/page.tsx`. `useAutostart` cae a `'unsupported'` y esconde la sección si `isTauri()` da false o si el plugin tira error. Desbloqueado: `v0.13.0` ya trae el logging, así que se diagnostica instalando ese build — sin shippear nada primero. |
